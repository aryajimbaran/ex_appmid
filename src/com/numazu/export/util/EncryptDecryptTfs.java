...
public class EncryptDecryptTfs {
	...
	public EncryptDecryptTfs() throws NoSuchAlgorithmException, NoSuchPaddingException {
	 ...
	 * @throws IOException 
	 */
	 	public String encryptDecrypt(String _inputText, String _encryptionKey,
			EncryptMode _mode, String _initVector) throws InvalidKeyException, InvalidAlgorithmParameterException,
		...
		if (_mode.equals(EncryptMode.ENCRYPT)) {
			...
			byte[] results = _cx.doFinal(_inputText.getBytes("UTF-8")); // Finish
										...
										// transformation
										// (encryption)
			_out = Base64.encodeBytes(results); // ciphertext
										// output
		}
		...
		return _out; // return encrypted/decrypted string
	 	...
	 	public String encryptDecryptAsBytes(byte[] _inputText, String _encryptionKey,
				EncryptMode _mode, String _initVector) throws InvalidKeyException, InvalidAlgorithmParameterException,
				IllegalBlockSizeException, BadPaddingException, IOException {
			String _out = "";// output string
			//_encryptionKey = md5(_encryptionKey);
			...
			if (_mode.equals(EncryptMode.ENCRYPT)) {
				...
				// for more info.
				_cx.init(Cipher.ENCRYPT_MODE, keySpec, ivSpec);// Initialize this cipher instance
				byte[] results = _cx.doFinal(_inputText); // Finish
											// multi-part
											// transformation
											// (encryption)
				_out = Base64.encodeBytes(results); // ciphertext
	 	...
	 	public byte[] decryptAsBytes(String _inputText, String _encryptionKey,
				EncryptMode _mode, String _initVector) throws InvalidKeyException, InvalidAlgorithmParameterException,
				IllegalBlockSizeException, BadPaddingException, IOException {
			//_encryptionKey = md5(_encryptionKey);
			//System.out.println("key="+_encryptionKey);
			...
			if (_mode.equals(EncryptMode.DECRYPT)) {
				...
				byte[] decodedValue = Base64.decode(_inputText);
				decryptedVal = _cx.doFinal(decodedValue); // Finish
										// multi-part
										// transformation
										// (decryption)
			}
			return decryptedVal; // return encrypted/decrypted string
		}
